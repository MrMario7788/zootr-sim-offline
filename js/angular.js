...
$scope.peekAt = function(loc_name) {
  ...
  if (loc.type == 'Warp') {
    ...
    if (destination.region) {
      hintItem = $scope.getRegion(destination.region)
    } else {
      hintItem = $scope.getRegion(destination)
    }
...
$scope.getPrice = function(loc_name) {
  loc = locationsByName[loc_name]
  if (loc.type == 'Shop') {
    return $scope.shopPrices[loc_name]
  }
  return null
}
  ...
  $scope.takeEntrance = function(entrance) {
    ...
    if (entrance == 'Pull Master Sword') {
      ...
      if (entranceInfo) {
        ...
        $scope.currentRegion = $scope.getRegion(destination)
  ...
  $scope.parseLog = function(logfile) {
    ...
    try {
      ...
      for (var loc_name in results) {
        ...
        if (typeof results[loc_name] == 'object') {
          $scope.shopPrices[loc_name] = results[loc_name]['price'];
        }
        ...
        if ($scope.entrances[entrance.checkName]) {
          ...
          if (typeof(destinationData) == 'object') {
            ...
            if (!(leaveDestination in entrancesByCheckName)) {
              var destinationFromRegion = $scope.getRegion(destinationData.from)
